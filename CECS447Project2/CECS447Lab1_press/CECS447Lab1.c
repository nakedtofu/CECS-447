...
void PortC_Init(void){
  ...
  GPIO_PORTC_PUR_R |= 0x70;     //     enable weak pull-up on PC4-6
	
	GPIO_PORTC_IS_R &= ~0x70;     // (d) PF0,4 is edge-sensitive
  GPIO_PORTC_IBE_R &= ~0x70;    //     PF0,4 is not both edges
  GPIO_PORTC_IEV_R &= ~0x70;    //     PF0,4 falling edge event
  GPIO_PORTC_ICR_R = 0x70;      // (e) clear flags 0,4
  GPIO_PORTC_IM_R |= 0x70;      // (f) arm interrupt on PF0,4
  NVIC_PRI0_R |= (NVIC_PRI7_R&0xFF0FFFFF)|0x00400000; // (g) priority 2
...
void PortD_Init(void){
  ...
  GPIO_PORTD_PUR_R |= 0x0F;     //     enable weak pull-up on PD0-3
	
	GPIO_PORTD_IS_R &= ~0x0F;     // (d) PF0,4 is edge-sensitive
  GPIO_PORTD_IBE_R &= ~0x0F;    //     PF0,4 is not both edges
  GPIO_PORTD_IEV_R &= ~0x0F;    //     PF0,4 falling edge event
  GPIO_PORTD_ICR_R = 0x0F;      // (e) clear flags 0,4
  GPIO_PORTD_IM_R |= 0x0F;      // (f) arm interrupt on PF0,4
  NVIC_PRI0_R |= (NVIC_PRI7_R&0x0FFFFFFF)|0x40000000; // (g) priority 2
  NVIC_EN0_R |= 0x00000008;      // (h) enable interrupt 2 in NVIC
...
//port D handler
void GPIOPortD_Handler() // called on touch of SW2
{ 
	
  if(GPIO_PORTD_RIS_R&0x01) // SW2 button
	{  
	...
	if(GPIO_PORTD_RIS_R&0x02) // SW1 button
	{  
	...
	if(GPIO_PORTD_RIS_R&0x04) // SW2 button
	{  
	...
	if(GPIO_PORTD_RIS_R&0x08) // SW1 button
	{  
...
//port C handler
void GPIOPortC_Handler() // called on touch of SW2
{
	
  if(GPIO_PORTC_RIS_R&0x10) // SW2 button
	{  
	...
	if(GPIO_PORTC_RIS_R&0x20) // SW1 button
	{  
	...
	if(GPIO_PORTC_RIS_R&0x40) // SW1 button
	{  
